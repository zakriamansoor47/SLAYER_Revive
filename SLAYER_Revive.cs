...
public class SLAYER_Revive : BasePlugin, IPluginConfig<SLAYER_ReviveConfig>
{
    ...
    public override void Load(bool hotReload)
    {
        RegisterListener<Listeners.OnTick>(() =>
        {
            ...
            foreach (var player in Utilities.GetPlayers().Where(player => player != null && player.IsValid && player.Connected == PlayerConnectedState.PlayerConnected && player.TeamNum > 1 && player.Pawn.Value.LifeState == (byte)LifeState_t.LIFE_ALIVE&& !player.IsHLTV && !player.IsBot && IsPlayerReviving != null && IsPlayerReviving.ContainsKey(player)))
            {
